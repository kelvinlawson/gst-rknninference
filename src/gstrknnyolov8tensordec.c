...
/* Pass video through in place; accept DMA-BUF or system memory. */
    ...
    GstCaps *incaps, GstCaps *outcaps)
{
   ...
   * frame dimensions (to scale normalised bboxes to pixels). Modern DMA_DRM
   * caps can't be read by gst_video_info_from_caps(), so resolve them first. */
#if GST_CHECK_VERSION (1, 24, 0)
